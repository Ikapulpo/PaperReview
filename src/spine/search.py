...
SPINE_JOURNALS_QUERY = (
    ...
    "AND (spine OR spinal OR vertebral OR lumbar OR cervical OR thoracic "
    "OR scoliosis OR kyphosis OR intervertebral OR fusion))"
)
...
class SpinePaper:
    ...
    def to_dict(self) -> dict:
        d = asdict(self)
        d["url"] = self.url
        d["first_author"] = self.first_author
        d["first_affiliation"] = self.first_affiliation
        d["journal_citation"] = self.journal_citation
        return d


class SpineSearchClient:
    """Client for searching spine journal papers on PubMed."""
    ...
    def search_and_fetch(self, days: int = 7) -> list[SpinePaper]:
        """Search for recent spine journal papers and fetch their details."""
