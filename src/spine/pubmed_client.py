...
JNS: Spine, Global Spine Journal, and JBJS (spine-related only).
"""
...
SPINE_SEARCH_QUERY = (
    '"Spine (Phila Pa 1976)"[ta] OR '
    '"Spine J"[ta] OR '
    '"Eur Spine J"[ta] OR '
    '"J Neurosurg Spine"[ta] OR '
    '"Global Spine J"[ta] OR '
    '("J Bone Joint Surg Am"[ta] AND '
    "(spine[tiab] OR spinal[tiab] OR vertebr*[tiab] OR lumbar[tiab] OR "
    '"cervical spine"[tiab] OR "thoracic spine"[tiab] OR scoliosis[tiab] OR '
    "kyphosis[tiab] OR lordosis[tiab] OR intervertebral[tiab] OR "
    "laminectomy[tiab] OR laminoplasty[tiab] OR "
    '"spinal fusion"[tiab] OR "spinal stenosis"[tiab] OR '
    "myelopathy[tiab] OR radiculopathy[tiab] OR "
    '"disc herniation"[tiab] OR discectomy[tiab]))'
)


@dataclass
class SpinePaper:
    """Represents a PubMed paper from a spine journal."""
...
class SpinePubMedClient:
    ...
    def search_recent(self, days: int = 7) -> list[str]:
        ...
        logger.info(
            f"PubMed search: found {total} spine papers, retrieved {len(id_list)} PMIDs"
        )
        return id_list
