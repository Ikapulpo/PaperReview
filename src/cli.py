...
def main():
    parser = argparse.ArgumentParser(
        description="週刊論文レビューシステム（NKT + Spine）",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
使用例:
  paper-review run                  NKT: 今すぐ実行
  paper-review spine run            Spine: 今すぐ実行
  paper-review spine run --days 14  Spine: 過去14日間
  paper-review spine search         Spine: 検索のみ（Notion投稿なし）
  paper-review spine schedule       Spine: 毎週自動実行スケジューラ起動
  paper-review spine verify         Spine: Notion接続テスト
  paper-review schedule             NKT: 毎週自動実行スケジューラ起動
""",
    )
    ...
    p_schedule = subparsers.add_parser("schedule", help="NKT: スケジューラ起動")
    ...
    p_spine_run = spine_sub.add_parser("run", help="Spine論文レビューを実行")
    ...
    p_spine_search = spine_sub.add_parser("search", help="Spine: PubMed検索のみ")
    ...
    p_spine_schedule = spine_sub.add_parser("schedule", help="Spine: スケジューラ起動")
