...
def cmd_spine_schedule(args):
    """Start the spine-only scheduler."""
...
def cmd_spine_verify(args):
    ...
    try:
        ...
        if client.verify_connection():
            print("✓ Notion接続成功 (スパイン)")
        else:
            print("✗ Notion接続失敗 (スパイン)")
            sys.exit(1)
    except ValueError as e:
        print(f"✗ 設定エラー: {e}")
        sys.exit(1)
...
使用例:
  ...
  paper-review spine schedule         脊椎スケジューラのみ起動
  paper-review spine verify           脊椎Notion接続テスト
  paper-review schedule               全スケジューラ起動 (NKT + スパイン)
...
""",
    ...
    # schedule (all)
    p_schedule = subparsers.add_parser("schedule", help="全スケジューラ起動 (NKT + スパイン)")
    ...
    # spine schedule
    p_spine_sched = spine_sub.add_parser("schedule", help="脊椎スケジューラのみ起動")
    p_spine_sched.set_defaults(func=cmd_spine_schedule)

    # spine verify
    p_spine_verify = spine_sub.add_parser("verify", help="脊椎Notion接続テスト")
