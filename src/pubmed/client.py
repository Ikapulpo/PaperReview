"""PubMed E-utilities API client for NKT cell paper search."""

import logging
import time
import xml.etree.ElementTree as ET
from dataclasses import dataclass, field
from datetime import datetime, timedelta

import requests

from src.config import config

logger = logging.getLogger(__name__)

# Base NKT search query targeting mouse and human studies
NKT_SEARCH_QUERY = (
    '("NKT cell" OR "NKT cells" OR "natural killer T cell" OR '
    '"natural killer T cells" OR "iNKT" OR "invariant NKT" OR '
    '"Vα14" OR "Valpha14" OR "Vα24" OR "Valpha24" OR '
    '"CD1d-restricted" OR "CD1d restricted") '
    'AND ("mouse" OR "mice" OR "murine" OR "human" OR "patient" OR "clinical")'
)


@dataclass
class Paper:
    """Represents a PubMed paper."""
    pmid: str
    title: str
    abstract: str
    authors: list[str]
    journal: str
    pub_date: str
    doi: str = ""
    keywords: list[str] = field(default_factory=list)
    mesh_terms: list[str] = field(default_factory=list)
    affiliation: str = ""
    volume: str = ""
    issue: str = ""

    @property
    def url(self) -> str:
        return f"https://pubmed.ncbi.nlm.nih.gov/{self.pmid}/"

    @property
    def first_author(self) -> str:
        return self.authors[0] if self.authors else "Unknown"


class PubMedClient:
    """Client for PubMed E-utilities API."""

    def __init__(self):
        self.base_url = config.pubmed_base_url
        self.api_key = config.ncbi_api_key
        self.session = requests.Session()

    def _build_params(self, **kwargs) -> dict:
        params = dict(kwargs)
        if self.api_key:
            params["api_key"] = self.api_key
        return params

    def _request_with_retry(self, url: str, params: dict, max_retries: int = 3) -> requests.Response:
        for attempt in range(max_retries):
            try:
                resp = self.session.get(url, params=params, timeout=30)
                resp.raise_for_status()
                return resp
            except requests.RequestException as e:
                if attempt < max_retries - 1:
                    wait = 2 ** (attempt + 1)
                    logger.warning(f"Request failed (attempt {attempt + 1}), retrying in {wait}s: {e}")
                    time.sleep(wait)
                else:
                    raise

    def search_recent(self, days: int = 7, query: str | None = None, batch_size: int | None = None) -> list[str]:
        """Search PubMed for papers published in the last N days.

        Returns list of PMIDs.
        """
        date_from = (datetime.now() - timedelta(days=days)).strftime("%Y/%m/%d")
        date_to = datetime.now().strftime("%Y/%m/%d")

        params = self._build_params(
            db="pubmed",
            term=query or NKT_SEARCH_QUERY,
            datetype="pdat",
            mindate=date_from,
            maxdate=date_to,
            retmax=batch_size or config.pubmed_search_batch,
            retmode="json",
            sort="relevance",
        )

        resp = self._request_with_retry(f"{self.base_url}/esearch.fcgi", params)
        data = resp.json()

        id_list = data.get("esearchresult", {}).get("idlist", [])
        total = data.get("esearchresult", {}).get("count", "0")
        logger.info(f"PubMed search: found {total} papers, retrieved {len(id_list)} PMIDs")
        return id_list

    def fetch_details(self, pmids: list[str]) -> list[Paper]:
        """Fetch detailed information for a list of PMIDs."""
        if not pmids:
            return []

        papers = []
        # Process in batches of 50
        batch_size = 50
        for i in range(0, len(pmids), batch_size):
            batch = pmids[i:i + batch_size]
            batch_papers = self._fetch_batch(batch)
            papers.extend(batch_papers)
            if i + batch_size < len(pmids):
                time.sleep(0.5)  # Rate limiting

        return papers

    def _fetch_batch(self, pmids: list[str]) -> list[Paper]:
        """Fetch details for a batch of PMIDs using efetch."""
        params = self._build_params(
            db="pubmed",
            id=",".join(pmids),
            retmode="xml",
        )

        resp = self._request_with_retry(f"{self.base_url}/efetch.fcgi", params)
        return self._parse_xml(resp.text)

    def _parse_xml(self, xml_text: str) -> list[Paper]:
        """Parse PubMed XML response into Paper objects."""
        papers = []
        try:
            root = ET.fromstring(xml_text)
        except ET.ParseError as e:
            logger.error(f"Failed to parse PubMed XML: {e}")
            return papers

        for article in root.findall(".//PubmedArticle"):
            try:
                paper = self._parse_article(article)
                if paper:
                    papers.append(paper)
            except Exception as e:
                pmid = article.findtext(".//PMID", "unknown")
                logger.warning(f"Failed to parse article PMID={pmid}: {e}")

        return papers

    def _parse_article(self, article) -> Paper | None:
        """Parse a single PubmedArticle XML element."""
        pmid = article.findtext(".//PMID", "")
        if not pmid:
            return None

        # Title
        title = article.findtext(".//ArticleTitle", "")

        # Abstract
        abstract_parts = []
        for abs_text in article.findall(".//AbstractText"):
            label = abs_text.get("Label", "")
            text = "".join(abs_text.itertext())
            if label:
                abstract_parts.append(f"{label}: {text}")
            else:
                abstract_parts.append(text)
        abstract = " ".join(abstract_parts)

        # Authors
        authors = []
        for author in article.findall(".//Author"):
            last = author.findtext("LastName", "")
            fore = author.findtext("ForeName", "")
            if last:
                authors.append(f"{last} {fore}".strip())

        # Journal
        journal = article.findtext(".//Journal/Title", "")

        # Publication date
        pub_date_elem = article.find(".//PubDate")
        if pub_date_elem is not None:
            year = pub_date_elem.findtext("Year", "")
            month = pub_date_elem.findtext("Month", "")
            day = pub_date_elem.findtext("Day", "")
            pub_date = f"{year} {month} {day}".strip()
        else:
            pub_date = ""

        # DOI
        doi = ""
        for id_elem in article.findall(".//ArticleId"):
            if id_elem.get("IdType") == "doi":
                doi = id_elem.text or ""
                break

        # Affiliation (first author)
        affiliation = ""
        first_author_elem = article.find(".//Author")
        if first_author_elem is not None:
            aff_elem = first_author_elem.find("AffiliationInfo/Affiliation")
            if aff_elem is not None and aff_elem.text:
                affiliation = aff_elem.text

        # Volume and Issue
        volume = article.findtext(".//JournalIssue/Volume", "")
        issue_num = article.findtext(".//JournalIssue/Issue", "")

        # Keywords
        keywords = [
            kw.text for kw in article.findall(".//Keyword")
            if kw.text
        ]

        # MeSH terms
        mesh_terms = [
            mh.findtext("DescriptorName", "")
            for mh in article.findall(".//MeshHeading")
            if mh.findtext("DescriptorName")
        ]

        return Paper(
            pmid=pmid,
            title=title,
            abstract=abstract,
            authors=authors,
            journal=journal,
            pub_date=pub_date,
            doi=doi,
            keywords=keywords,
            mesh_terms=mesh_terms,
            affiliation=affiliation,
            volume=volume,
            issue=issue_num,
        )

    def search_and_fetch(self, days: int = 7, query: str | None = None, batch_size: int | None = None) -> list[Paper]:
        """Search for recent papers and fetch their details."""
        pmids = self.search_recent(days=days, query=query, batch_size=batch_size)
        if not pmids:
            logger.info("No new papers found.")
            return []
        papers = self.fetch_details(pmids)
        logger.info(f"Fetched details for {len(papers)} papers")
        return papers
